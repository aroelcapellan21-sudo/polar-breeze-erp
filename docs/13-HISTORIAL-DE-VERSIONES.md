...
| v0.28 (MAYOR) | 2026-07-14 | — (este commit) | Se fija la jerarquía documental en `02-CONSTITUCION-ERP.md`: la Constitución prevalece ante cualquier conflicto, incluido con `01-VISION-ERP.md` (que queda como origen/fundamento, no instancia superior de apelación). Se elimina la frase "superada únicamente por `01-VISION-ERP.md`" del Objetivo. Decisión tomada por el usuario tras consulta explícita; no fue necesario modificar `01` ni `99-FILOSOFIA-DEL-SISTEMA.md`, ya compatibles. |
| v0.29 | 2026-07-14 | — (este commit) | Se formaliza como definitiva la coexistencia de `00-MANIFIESTO-DEL-ERP.md` y `00-PRINCIPIOS-DEL-ERP.md` bajo el prefijo `00` (nivel fundacional compartido por diseño), cerrando la excepción que estaba "a la espera de decisión del usuario" desde v0.23. Decisión tomada por el usuario tras consulta explícita. Ningún archivo se renombra. |
