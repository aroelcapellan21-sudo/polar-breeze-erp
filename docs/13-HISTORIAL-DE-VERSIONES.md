...
| v0.32 | 2026-07-14 | — (este commit) | Se elimina el campo duplicado `número` de `Cuenta` en `11-DICCIONARIO-DE-DATOS.md`: el campo `código` heredado queda como única clave de negocio, con su rango acotado a "1"-"6". `05-MODELO-DE-DATOS-MAESTRO.md` ya no requería cambios, pues siempre describió a `Cuenta` con un único campo. |
