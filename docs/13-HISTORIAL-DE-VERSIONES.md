...
| v0.42 | 2026-07-17 | `3d6dc0e` | Se define el proceso de aprobación formal de la documentación, cerrando el pendiente #8 de `resumenes/2026-07-14-estado-y-pendientes-para-retomar.md`. Doble autoridad: el Arquitecto/Product Owner del ERP aprueba documentos técnicos/estructurales, Oliver (dueño de Polar Breeze) aprueba documentos de negocio, y ambos aprueban los documentos que mezclan las dos naturalezas. Aprobación de todo el baseline vigente (00 a 14 y 99, salvo `06` y `DECISIONES-ARQUITECTURALES.md`, excluidos) como un solo evento con fecha 2026-07-17 sobre `v0.41`; una re-aprobación individual solo se dispara cuando un documento reciba un cambio de versión MAYOR después de esta fecha, nunca por cambios MENOR. Se agrega la sección "Registro de Aprobaciones" más abajo, y se actualiza el campo Estado de los 16 documentos aprobados. |
| v0.43 (MAYOR) | 2026-07-17 | — (este commit) | Se formaliza en `02-CONSTITUCION-ERP.md` el **Artículo 31 — Proceso de Aprobación Formal de la Documentación** (31.1 a 31.6), cerrando el pendiente #11 de `resumenes/2026-07-17-estado-y-pendientes-para-retomar.md`. Eleva a regla constitucional lo ya decidido en v0.42: doble autoridad, aprobación por baseline, re-aprobación solo ante cambios MAYOR, y la distinción entre esta aprobación documental y la inmutabilidad de documentos de negocio del Artículo 14. La Constitución pasa de 30 a 31 artículos y, por ser un cambio MAYOR, requiere su propia re-aprobación individual (ver Registro de Aprobaciones). |
| v0.44 | 2026-07-23 | `753eab4` | Se agrega el **Módulo 7 — Asistente de Consulta** a `08-CATALOGO-DE-MODULOS.md`: capa de consulta en lenguaje natural, estrictamente de solo lectura sobre las proyecciones y el historial de eventos del Motor de Flujos Patrimoniales, sin emitir eventos ni alterar estado patrimonial (Artículo 26 de la Constitución). Sigue la misma estructura de cinco bloques exigida por el Artículo 29.2. Quedan pendientes, sin resolver: si sus consultas se registran en el historial de auditoría (Artículo 8), y el canal de interacción concreto. El catálogo pasa de 6 a 7 módulos. Decisión registrada en `DECISIONES-ARQUITECTURALES.md`. |
| v0.45 | 2026-07-23 | `e88bd3e` | Se resuelven los dos pendientes del Módulo 7 — Asistente de Consulta en `08-CATALOGO-DE-MODULOS.md`: (a) las consultas del asistente no entran al historial de auditoría patrimonial del Artículo 8, sino a una bitácora de actividad propia y separada, sin naturaleza contable; (b) el asistente se diseña como servicio central consumible desde múltiples canales (chat en el Hub, comando de voz, otros futuros no descartados). Quedan dos pendientes nuevos, más acotados: formato/ubicación de la bitácora, y prioridad de implementación entre canales. Decisión registrada en `DECISIONES-ARQUITECTURALES.md`. |
| v0.46 | 2026-07-23 | `9738912` | Se precisa, en el bloque "Registro de actividad" del Módulo 7 (`08-CATALOGO-DE-MODULOS.md`), la redacción sobre el alcance de la bitácora de actividad frente al Artículo 8: se reformula para dejar explícito que su propósito es operativo y de seguridad, no patrimonial, y que por ello no está sujeta a las garantías de inmutabilidad exigidas para la auditoría patrimonial — sin cambiar el fondo ya decidido en v0.45. |
| v0.47 | 2026-07-23 | `a311c77` | Se formaliza el mecanismo de eventos compensatorios en `04-MOTOR-DE-FLUJOS-PATRIMONIALES.md`: la sección 3 (estructura mínima de un evento) gana los campos `eventoCorregidoId` (opcional), `motivoCorreccion` (obligatorio cuando el anterior está presente) y `tipoCompensacion` (opcional); la sección 8 aclara que ningún tipo de evento nuevo se crea para correcciones — cualquier evento del catálogo formal puede actuar como compensatorio mediante estos metadatos. Se actualiza en consecuencia la definición de "Evento compensatorio" en `12-GLOSARIO.md`, sección A. Decisión registrada en `DECISIONES-ARQUITECTURALES.md`. |
| v0.48 | 2026-07-23 | `f039808` | Se agrega la **Fase 7 — Módulo 7: Asistente de Consulta** a `10-PLAN-MAESTRO-DE-IMPLEMENTACION.md`, dependiente de la Fase 6 completa, cerrando el vacío de implementación que dejó ese módulo al aprobarse después del baseline v0.41. Se renumeran las fases posteriores (Validación Multiempresa Real → Fase 8, Crecimiento Continuo → Fase 9) y todos los encabezados de sección subsiguientes (Gobernanza del Plan, Riesgos Conocidos, Relación con Otros Documentos). Se corrige la referencia "antes de la Fase 7" en Riesgos Conocidos a "antes de la Fase 8" (inconsistencia detectada por la renumeración, confirmada con el usuario), y se actualiza la mención al catálogo de módulos de 6 a 7 en Relación con Otros Documentos. El plan pasa de 8 a 9 fases. Decisión registrada en `DECISIONES-ARQUITECTURALES.md`. |
| v0.49 | 2026-07-23 | `4b44903` | Se crea el anexo `docs/anexos/03-EJEMPLOS-MOTOR-DE-FLUJOS.md`: tres casos completos de principio a fin (compra a proveedor, consignación con liquidación, corrección de evento) que ilustran, sin agregar reglas nuevas, la aplicación práctica del catálogo formal de 22 eventos de `12-GLOSARIO.md`. Se confirma el uso exclusivo de `ObligacionRegistrada` para deudas de la empresa con terceros (nunca crédito a clientes), y se valida en la práctica el mecanismo de eventos compensatorios de v0.47. Se enlaza el anexo desde `docs/anexos/README.md`. Decisión registrada en `DECISIONES-ARQUITECTURALES.md`. |
| v0.50 | 2026-07-23 | `ef0269d` | Corrección de redacción en `docs/anexos/03-EJEMPLOS-MOTOR-DE-FLUJOS.md`: se retira la cifra fija "22 eventos" del bloque Estado y de la sección Relación con Otros Documentos, dejando la referencia al catálogo formal de `12-GLOSARIO.md`, sección C, sin conteo numérico — evita que el anexo quede desactualizado si el catálogo de eventos crece. No modifica ninguna regla ni contenido normativo. |
...
| `02-CONSTITUCION-ERP.md` (re-aprobación) | Arquitecto/Product Owner + Oliver | 2026-07-17 | v0.43 — cambio MAYOR (Artículo 31 agregado), dispara re-aprobación individual según el Artículo 31.4 |
