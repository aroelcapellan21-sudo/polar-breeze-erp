...
| v0.27 | 2026-07-14 | — (este commit) | Se eleva `08-CATALOGO-DE-MODULOS.md` al estándar del Artículo 29.2 de la Constitución: cada uno de los cinco módulos declara ahora su alcance por `empresaId`/`sucursalId`, los eventos que emite y los de otros módulos que le afectan, y los catálogos maestros que crea o consume. Estado del documento pasa de "En construcción" a "Vigente", resolviendo la inconsistencia con su fila en la tabla de completitud de este mismo historial. Se ajustan las entradas de `Cliente` y `Proveedor` en `05` para declarar su módulo de creación. |
